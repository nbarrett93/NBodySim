...
CL_Components get_CL_components(bool &success)
{
	success = false;
	CL_Components ret;
	...
	for (cl_uint i = 0; i < num_platforms; ++i)
	{
		...
		if (num_devices != 0)
		{
			...
			{
				if (device_ids != NULL) delete[] device_ids;
				if (platform_ids != NULL) delete[] platform_ids;
				return CL_Components();
			}

			// default device (hopefully a good one!)
			ret.device = device_ids[0];
			ret.platform = platform_ids[i];
...
{
	cl_int err;
	cl_context ctx = clCreateContext(properties, 1, &comp.device, NULL, NULL, &err);

	if (err == CL_SUCCESS)
	{
		cl_command_queue queue = clCreateCommandQueue(ctx, comp.device, NULL, &err);
